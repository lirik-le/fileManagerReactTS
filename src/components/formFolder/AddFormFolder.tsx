...
import {useAddFolderMutation, useGetFolderQuery} from "../../redux/slice/folders/folderApiSlice";
...
const AddFormFolder: FC<Props> = ({setActiveModal}) => {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            await addFolderMutation({name, parentId}).unwrap();
            setName('');
            setActiveModal('');
            refetch();
        } catch (error) {
        ...
        <div
            ...
            <div className="w-full max-w-md bg-white rounded-lg shadow-md p-6 relative">
                ...
                <form className="flex flex-col" onSubmit={handleSubmit}>
                    <p className='mb-5 text-xl'>Создание папки</p>
                    <input type='text'
                    ...
                    />
                    <button type="submit"
                            className="button-class">Создать
                    </button>
                </form>
            </div>
        </div>
